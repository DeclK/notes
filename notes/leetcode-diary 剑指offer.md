...
2. 对于 `left = mid` 这样的缩小范围的方式，由于整除的下取整性质，可能会有 `mid == left`，这将会陷入死循环。所以要保证左侧往前走 `left = mid + 1`，这样才能使得 left == right，跳出循环完成搜索
3. 最优解（如果存在）尽可能在左右指针的范围之内。即：在二分搜索的时候 `right = mid - 1` 中的 1 有时是需要考虑的
