...
经过 deepseek 的总结，有了更加清晰的表格

| **场景**         | **语法示例**                   | **说明**                       |
| ---------------- | ------------------------------ | ------------------------------ |
| 访问嵌套类型     | `Outer::Inner`                 | 类、枚举、`typedef` 等嵌套类型 |
| 访问静态成员     | `MyClass::static_var`          | 无需实例化对象                 |
| 类外定义成员函数 | `void MyClass::func() { ... }` | 将函数实现绑定到类作用域       |
| 访问命名空间成员 | `MyNamespace::value`           | 命名空间内变量/函数            |
| 强制访问全局变量 | `::x`                          | 避免局部变量覆盖全局变量       |
| 模板中的依赖类型 | `typename T::NestedType`       | 声明模板参数中的嵌套类型       |

需要进一步说明的点：

1. 所谓嵌套类型，就是在类中定义的类。
2. 模版中的依赖类型，其是就是模版中的嵌套类型，要使用模版类中的类，仍然使用 `::` 进行解析，不一样的是，需要使用 `typename` 来声明这个嵌套类型是一个类，而不是类的实例化对象或其他静态成员变量。这都是由模板所带来的不确定性导致的

由此对于 `::` 的用法就变得非常清晰了，只要跟类相关的访问、解析，基本上都考虑使用 `::`，与此同时，还可以将 namespace 抽象为一种特殊的类，因为 namespace 也使用 `::` 进行解析，并且也可使用 `using namespace xxx` 来进行指定

```cpp
namespace ≈ 一个不可实例化、仅有公有静态成员的类（但没有访问控制，且可自由扩展）
```

所以通过 `::` 去访问 namespace 下的成员就变得更容易理解了
