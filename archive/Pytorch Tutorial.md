...
1. `input & index` 他们的维度数量是一样的，`index` 的维度长度可以小于 `input`

2. `gather` 是在某一个维度进行的，可以想象固定其他维度不动，在某一个 axis 上面选择一部分 index

3. `index` 和 `gather output` 形状是一样的，同理，scatter 中 index 和 source 的形状是一致的。因为 `gather` 和 `scatter` 为一个逆向操作  

   <img src="Pytorch Tutorial/image-20230407220109016.png" alt="image-20230407220109016" style="zoom: 50%;" />
